...
const [lossHistory, setLossHistory] = useState<{ loss: number; metric: number; val_loss?: number; val_metric?: number }[]>([]);
...
lossHistory: [] as { loss: number; metric: number; val_loss?: number; val_metric?: number }[],
outputs: [] as number[],
  });
...
const shuffleData = (inputs: number[][], outputs: number[][]): { inputs: number[][]; outputs: number[][] } => {
...
const normalize = (data: number[][], min: number[], max: number[]): number[][] => {
return data.map(row =>
row.map((val, idx) => {
...
const denormalize = (data: number[][], min: number[], max: number[]): number[][] => {
return data.map(row =>
row.map((val, idx) => {
const range = max[idx] - min[idx];
...
const computeMinMax = (data: number[][]): { min: number[]; max: number[] } => {
...
let layerActivation = layerIdx === layerSizes.length - 2
? (problemType === "Regression" ? "linear" : (outputNeurons > 1 ? "softmax" : "sigmoid"))
: activationFunction;
...
alert(`Dataset inputs have shape [${dataset.inputs.length}, ${dataset.inputs[0].length}], but model expects [null, ${inputNeurons}]. Please reload the dataset.`);
return;
    }
if (dataset.inputs.some(row => row.some(val => isNaN(val))) || dataset.outputs.some(row => row.some(val => isNaN(val)))) {
...
alert("Empty or invalid dataset shape after preprocessing. Please check your selected columns and data.");
xs.dispose();
ys.dispose();
return;
    }
...
model.add(tf.layers.dense({ units, activation: activationFunction as any, kernelInitializer: 'glorotNormal' }))
    );
const outputActivation = problemType === "Regression" ? undefined : (outputNeurons > 1 ? "softmax" : "sigmoid");
...
console.warn("Dataset too small for validation split. No validation metrics will be available.");
    }
...
const lossValue = typeof history.history.loss?.[0] === 'number' ? history.history.loss[0] : (history.history.loss?.[0] as any)?.dataSync?.()?.[0] ?? 0;
console.log(`Epoch ${epoch} loss: ${lossValue.toFixed(4)}`);
const valLossRaw = history.history.val_loss?.[0] ?? undefined;
const valLoss = typeof valLossRaw === 'number' ? valLossRaw : (valLossRaw as any)?.dataSync?.()?.[0] ?? undefined;
...
if (problemType === "Regression") {
  ...
  const metricKeys = allKeys.filter(k => k !== 'loss' && !k.startsWith('val_'));
  const metricKey = metricKeys.length > 0 ? metricKeys[0] : 'acc';
  metricValue = (history.history[metricKey]?.[0] as number ?? 0);
  const valMetricKeys = allKeys.filter(k => k.startsWith('val_') && k !== 'val_loss');
  const valMetricKey = valMetricKeys.length > 0 ? valMetricKeys[0] : 'val_acc';
  valMetric = (history.history[valMetricKey]?.[0] as number ?? undefined);
}
setLoss(lossValue.toFixed(4));
setLossHistory((prev) => [...prev, { loss: lossValue, metric: metricValue, val_loss: valLoss, val_metric: valMetric }]);
...
console.log(`Epoch ${epoch} weight change sum: ${weightDiffSum.toFixed(6)}`);
...
await new Promise((resolve) => setTimeout(resolve, 300 / animationSpeedRef.current));
await playBackward({
model,
neuronValuesMap,
...
let currentWeights: number[][][] = JSON.parse(JSON.stringify(parsedWeights as number[][][]));
let currentBiases: number[][] = JSON.parse(JSON.stringify(parsedBiases as number[][]));
...
const weightsToNeuron = currentWeights[layer.from].map((row) => (row[step.toIdx] as number) ?? 0);
...
const { neuronValuesMap: valueMap, preTrainWeights, postTrainWeights, postTrainBiases } = epochLog;
const positions = getNeuronPositions();
...
const newDisplayedConnections = new Set<string>();
const newDisplayedWeights = new Map<string, number>();
let currentWeights: number[][][] = JSON.parse(JSON.stringify(preTrainWeights as number[][][]));
let currentBiases: number[][] = JSON.parse(JSON.stringify(postTrainBiases as number[][]));
...
if (progress >= 1) {
for (const step of layer.steps) {
const prevWeight = currentWeights[layer.from][step.fromIdx][step.toIdx];
currentWeights[layer.from][step.fromIdx][step.toIdx] = step.weight;
currentBiases[layer.to - 1][step.toIdx] = step.bias;
newDisplayedWeights.set(`${layer.from}-${step.fromIdx}-${step.toIdx}`, step.weight);
const gradientKey = `${layer.from}-${step.fromIdx}`;
const prevGradient = gradientMap.get(gradientKey) || 0;
gradientMap.set(gradientKey, prevGradient + Math.abs(step.weight - prevWeight));
const equationKey = `${layer.to}-${step.toIdx}`;
const weightsToNeuron = currentWeights[layer.from].map((row) => (row[step.toIdx] as number) ?? 0);
const bias = step.bias ?? 0;
const terms = weightsToNeuron.map((w, i) => {
...
alert(`Expected ${inputNeurons} inputs, but received ${rawInputs.length}.`);
return;
    }
...
const { neuronValuesMap } = await computeActivations(model, normalizedInputs);
await playForward({ model, neuronValuesMap, parsedWeights, parsedBiases });
setTrainingPhase("");
  };
const onDatasetUpload = (data: { inputs: number[][]; outputs: number[][]; needsOutputNormalization?: boolean }) => {
if (
...
setOutputMin([]);
setOutputMax([]);
    } else if (inputNeurons !== 3 || outputNeurons !== 1 || hiddenLayers.toString() !== '2') {
setInputNeurons(3);
setOutputNeurons(1);
setHiddenLayers([2]);
...
<p>No dataset loaded. Showing dummy network. Please load a dataset.</p>
</div>
          )}
